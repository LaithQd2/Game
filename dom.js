...
function attackMode(row, col) {
  if (!isGameOver()) {
    ...
    while (!isGameOver() && count < 2) {
      isPlayerTurn ? player.takeTurn(computerGameboard, row, col) : computer.takeTurn(playerGameboard);
...
function renderGameBoard(gameboard) {
  const gridSelector = isGameStarted ? getGridSelector(gameboard) : '.player-grid .cell';
  ...
  cells.forEach(cell => {
    ...
    isGameStarted ? handleCellForGameStarted(cell, value) : handleCellForGameNotStarted(cell, value);
  });
}

function getGridSelector(gameboard) {
  return gameboard === playerGameboard ? '.computer-grid .cell' : '.player-grid .cell';
}
